...
### 1) на вход идет адресс BHD (нет мы теперь парсим главную табличку)
# начинается на aa55aa55, заканчивается на 0055ffff
### 2) смотрим количество записей BIOS, там будут адреса
### 3) Размер записи с типом и адресом - 6*4 байт
# int тип
# int размер
# int адрес (учитывай оффсет бинаря!)
# 3*int ???? (обычно там нули и FFFFFF...)
### 4) По ним парсим че за файлы там лежат
### P.S. coreboot хуета
